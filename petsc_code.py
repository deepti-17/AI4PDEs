...
dt = 0.01 / 1.5   # ≈ 0.00667 dimensional time step

beta = 0.25                   
H_cells = int(D_cells / beta) # ny = 80
Xu_cells = int(45 * D_cells)  # Upstream = 45D
Xd_cells = int(120 * D_cells)  # Downstream = 120D

nx = Xu_cells + Xd_cells      # 45D + 120D = 165D = 3300 cells
ny = H_cells                  # 80 cells
ntime = 30000                 
...
for i in range(nx):
    for j in range(ny):
        dist2 = (i - cor_x)**2 + (j - cor_y)**2
        if dist2 <= radius**2 and i >= cor_x:   # i >= cor_x → curved half faces downstream
            mask[i, j] = 1.0
# Also add the flat face column at i == cor_x
mask[cor_x, cor_y-radius:cor_y+radius+1] = 1.0
...
#     u_prof = 1.5 * ramp * (1.0 - (1.0 - 2.0 * y_idx / (ny - 1))**2)
    y_idx = np.arange(ny)
    y_norm = y_idx / (ny - 1)          # maps 0..1 (= 0..H/D * D/H)
    u_prof = 1.5 * ramp * (1.0 - (np.abs(1.0 - 2.0 * y_norm))**2)
# Paper: Ux = 1.5·[1 − |1 − 2βy|²], β=0.25, y in [0, H/D]
# With β=0.25 and y in [0,4], βy in [0,1] → same as y_norm above ✓
...
probe_x = cor_x + int(2.0 * D_cells)   # 2D downstream (was 1.5D)
probe_y = cor_y + 3
...
#                 plot_end_x = min(nx, cor_x + int(4.0 * D_cells))
                plot_start_x = max(0, cor_x - int(1.0 * D_cells))
                plot_end_x   = min(nx, cor_x + int(5.0 * D_cells))  # extend to show more wake
                x_dim = np.linspace(44.0, 50.0, plot_end_x - plot_start_x)
