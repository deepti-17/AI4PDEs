...
Y, X = torch.meshgrid(
    torch.arange(ny, device=device) * dy,
    torch.arange(nx, device=device) * dx,
    indexing="ij"
)
...
class AI4Burgers(nn.Module):
    ...
    def forward(self, u, u_vel):
        ...
        Lu = (
            nu * self.diff(u_pad)
            - u_vel * self.xadv(u_pad)
            - u_vel * self.yadv(u_pad)
        )
...
def UNIT_TEST(c):
    ...
    var = torch.sum(c*((X-x0)**2 + (Y-y0)**2)) * dx * dy / mass
...
class PETScOperator:
    ...
    def mult(self, mat, x, y):
        ...
        x_torch = torch.from_numpy(
            x_np.copy()
        ).float().view(1,1,self.ny,self.nx).to(device)
...
def run_simulation():
    ...
    print(f"{'Step':<5} | {'Iter':<6} | {'Residual':<10} | {'Step Time':<10} | {'Mass':<10} | {'Max_Val':<10}")
    print("-"*70)

    mass,_,_,_ = UNIT_TEST(values_u)

    print(f"{0:<5} | {'---':<6} | {'---':<10} | {'---':<10} | {mass:<10.2f} | {values_u.max():<10.2f}")
    ...
    for t in range(1, ntime+1):
        ...
        current_u = torch.from_numpy(
            sol_vec.array
        ).view(1,1,ny,nx).to(device)
        ...
        if t%5==0 or t==1:
            ...
            print(
                f"{t:<5} | {its:<6} | {ksp.getResidualNorm():<10.2e} | {step_elapsed:<10.4f} | {mass:<10.2f} | {current_u.max():<10.2f}"
            )
    ...
    ax.set_title("Implicit Burgers' Equation")
